...
// 				if (isset($_GET['click_pro'])) {
// 					$error = "";
// 					$nameSearch = $_GET['search_pro'];

// 					if (empty($nameSearch)) {
// 						$error = "<p class='text-danger'> Bạn cần điền sản phẩm muốn tìm !! </p>";
// 					} 
// 					if (!empty($error)) {
// 						echo $error;
// 					} else {
// 						$query = "SELECT * FROM product WHERE name LIKE '%$nameSearch%' AND status = 1 ";
// 					}
...
<body class="animsition">
	...
	<div class="bg0 m-t-23 p-b-140">
		<div class="container">
			...
			if (!empty($_GET['search_pro'])) {
				$name = $_GET['search_pro'];
				$query = "SELECT * FROM product WHERE color LIKE '%$name%' || name LIKE '%$name%' || size LIKE '%$name%'  AND status = 1  ";

				$product = mysqli_query($conn, $query); ?>

				<div class="row isotope-grid">
					<?php
					while($pros = mysqli_fetch_assoc($product)) :
			...
			<?php } else { ?>
				<div class="row isotope-grid ">
					<h3 class="text-danger">Bạn chưa nhập sản phẩm muốn tìm!!!!</h3>
				</div>
			<?php } ?>
